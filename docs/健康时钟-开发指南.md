...
2. 按当前项目结构创建 `data/`、`providers/`、`presentation/`
3. 使用 Riverpod 管理状态
4. 使用 GoRouter 配置路由
...
### 4. OCR 识别失败
- 检查百度 OCR API Key 和 Secret Key 是否正确
- 检查图片 URL 是否可访问
- 检查图片格式是否支持（JPG/PNG）
- 检查图片大小是否超过限制
...
2. 在 Apple Developer / Xcode Signing & Capabilities 中开启 Sign in with Apple
3. 后端 `.env` 配置 `APPLE_CLIENT_ID=<iOS Bundle ID>`
4. 配置通知权限、麦克风权限和语音识别权限
5. 使用 Xcode 打包上传到 App Store Connect
